...
window.addEventListener("load",function()
{
  ...
  $("#lessOpacity").on("click",function(){
			GLOBAL.opacity=Math.min(0.95,Math.max(0.1,GLOBAL.opacity-0.1));Update();
	});
  $("#moreOpacity").on("click",function(){
			GLOBAL.opacity=Math.max(0.05,Math.min(0.95,GLOBAL.opacity+0.1));Update();
	});
  ...
  function loadLabelMaps()
	...
	function loadTreeViewData(){
		...
		$treeview.jstree({
			'core' : {
				'data' : {
					"url" : "js/bnatlasviewer/Chimp_atlas_tree.json",//mcqatlas_tree.json
...
function x3d_onload(){
	...
	$.ajax("chimp_atlas/FC/1.json").done(function(data){
			console.log(data);
			// data = $.parseJSON(data);
			// console.log(data);
...
function select_fiber(id){
	var fc_name = "chimp_atlas/FC/" + id + ".json";
